...
using HQCWeb.FW.Data.Common.Exceptions;
using HQCWeb.FW.Data.Common.Utilities;
using HQCWeb.FW.Data.Common.Utilities.Objects;
using HQCWeb.FW.Data.Common.Utilities.Objects.Members;
using HQCWeb.FW.Data.Common.Xml;
using HQCWeb.FW.Data.DataMapper.Configuration.Alias;
using HQCWeb.FW.Data.DataMapper.Configuration.Cache;
using HQCWeb.FW.Data.DataMapper.Configuration.Cache.Fifo;
using HQCWeb.FW.Data.DataMapper.Configuration.Cache.Lru;
using HQCWeb.FW.Data.DataMapper.Configuration.Cache.Memory;
using HQCWeb.FW.Data.DataMapper.Configuration.ParameterMapping;
using HQCWeb.FW.Data.DataMapper.Configuration.ResultMapping;
using HQCWeb.FW.Data.DataMapper.Configuration.Serializers;
using HQCWeb.FW.Data.DataMapper.Configuration.Sql;
using HQCWeb.FW.Data.DataMapper.Configuration.Sql.Dynamic;
using HQCWeb.FW.Data.DataMapper.Configuration.Sql.Dynamic.Elements;
using HQCWeb.FW.Data.DataMapper.Configuration.Sql.SimpleDynamic;
using HQCWeb.FW.Data.DataMapper.Configuration.Sql.Static;
using HQCWeb.FW.Data.DataMapper.Configuration.Statements;
using HQCWeb.FW.Data.DataMapper.MappedStatements;
using HQCWeb.FW.Data.DataMapper.MappedStatements.ArgumentStrategy;
using HQCWeb.FW.Data.DataMapper.MappedStatements.PropertyStrategy;
using HQCWeb.FW.Data.DataMapper.Scope;
using HQCWeb.FW.Data.DataMapper.TypeHandlers;
using Microsoft.SqlServer.Server;

#endregion

namespace HQCWeb.FW.Data.DataMapper.Configuration
{
    /// <summary>
    ...
    public class DomSqlMapBuilder
    {
        ...
        /// </summary>
        public const string DEFAULT_SQL_MAP_PATH = "SqlMapFiles";
        ...
        private ISqlMapper Build(XmlDocument document,DataSource dataSource, bool useConfigFileWatcher, bool isCallFromDao)
        {
            ...
            string strSqlMapConfigFilePath = string.Empty;

            strSqlMapConfigFilePath = System.Configuration.ConfigurationManager.AppSettings.Get("SQLMAPCONFIG_FILE_PATH");
            ...
            try
            {
                ...
                {
                    ValidateSchema( document.ChildNodes[1], strSqlMapConfigFilePath + "SqlMapConfig.xsd" );
                }
                Initialize();
        ...
        private void ValidateSchema( XmlNode section, string schemaFileName )
        {
            ...
            try
            {
                xsdFile = File.OpenRead(schemaFileName);
                ...
                if (xsdFile == null)
                {
                    // LKC RETURN
                    return;
                    // TODO: avoid using hard-coded value "HQCWeb.FW.Data.DataMapper"
                    // throw new ConfigurationException( "Unable to locate embedded resource [HQCWeb.FW.Data.DataMapper."+schemaFileName+"]. If you are building from source, verfiy the file is marked as an embedded resource.");
                }
        ...
        private void ConfigureSqlMapModule()
        {
            ...
            {
                ValidateSchema(_configScope.SqlMapModuleDocument.ChildNodes[1], strSqlMapConfigFilePath + "SqlMapModule.xsd");
            }
        ...
        private void ConfigureSqlMap()
        {
            ...
            DirectoryInfo di = new DirectoryInfo(strSqlMapConfigFilePath + DEFAULT_SQL_MAP_PATH);

            //DirectoryInfo di = new DirectoryInfo(AppDomain.CurrentDomain.BaseDirectory + DEFAULT_SQL_MAP_PATH);
            //DirectoryInfo di = new DirectoryInfo(System.Environment.CurrentDirectory + "\\" + DEFAULT_SQL_MAP_PATH);
            //DirectoryInfo di = new DirectoryInfo(Directory.GetCurrentDirectory() + "\\" + DEFAULT_SQL_MAP_PATH);
            //string strPath = System.Configuration.ConfigurationManager.AppSettings.Get("BasePath");
            //DirectoryInfo di = new DirectoryInfo(Path.GetFullPath(strPath) + "\\" + DEFAULT_SQL_MAP_PATH);
            //DirectoryInfo di = new DirectoryInfo(@"C:\JC02_TANDEM_PILOT_LINE_MES\SOURCE\Framework\DotNet4\HQCWeb.FW\DotNetLib\" + DEFAULT_SQL_MAP_PATH);
            ...
            if (_configScope.UseConfigFileWatcher)
            {
                ...
                {
                    string ressource = NodeUtils.ParsePropertyTokens(sqlMapNode.Attributes["resource"].Value, _configScope.Properties);

                    fi = di.GetFiles(ressource, SearchOption.AllDirectories);
            ...
            {
                ConfigWatcherHandler.AddFileToWatch(Resources.GetFileInfo(fi[i].FullName));
                sqlMapNode.Attributes["resource"].Value = fi[i].FullName;
                // Load the file 
                _configScope.SqlMapDocument = Resources.GetAsXmlDocument(sqlMapNode, _configScope.Properties);

                if (_configScope.ValidateSqlMap)
                {
                    ValidateSchema(_configScope.SqlMapDocument.ChildNodes[1], strSqlMapConfigFilePath + "SqlMap.xsd");
                }
        ...
        {
            return Assembly.GetExecutingAssembly().GetManifestResourceStream("HQCWeb.FW.Data.DataMapper." + schemaResourceKey); 
        }
