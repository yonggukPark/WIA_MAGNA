    ...
    e.AmRectangularChart = e.Class({
        ...
        measureMargins: function () {
            ...
            if (a = this.categoryAxis)
                b = this.getAxisBounds(a, f, k, e, b), f = Math.round(b.l), k = Math.round(b.r), e = Math.round(b.t), b = Math.round(b.b);
...
(function () {
    ...
    e.AmSerialChart = e.Class({
        ...
        drawChart: function () {
			...
			//console.log("2 draw");
			//this.ddClickEvent(this.set, this.chartData);
        ...
        updateMargins: function () {
            ...
            if (a = this.valueScrollbar)
                this.getScrollbarPosition(a, !this.rotate, this.valueAxes[0].position), this.adjustMargins(a,
                    !this.rotate)
        },
        updateScrollbars: function () {
        ...
        showZB: function () {
            ...
            1 && (a = !0);
            if (b = this.valueAxes)
                b = b[0], isNaN(b.relativeStart) || (0 !== e.roundTo(b.relativeStart, 3) && (a = !0), 1 != e.roundTo(b.relativeEnd, 3) && (a = !0));
                ...
                if (void 0 !== a) {
                    if (-1 != a.indexOf("[[category]]")) {
                        ...
                        if (this.categoryAxis.parseDates) {
                            ...
                            -1 != h.indexOf("fff") && (h = e.formatMilliseconds(h,
                                        g));
                            g = h
                        }
			...
			if (a) {
                ...
                if (a.valueLineBalloonEnabled)
                    for (l = 0; l < this.valueAxes.length; l++)
                        h = this.valueAxes[l], (f = h.balloon) || (f = {}), f = e.extend(f, this.balloon, !0), f.fillColor = d, f.balloonColor = d, f.fillAlpha = c, f.borderColor =
        ...
        handleCursorMove: function (a) {
            ...
            if (a.panning)
				...
				//console.log("chartData=", a.chartData);
				//console.log("isHidden=", b.isHidden);
                    ...
                    if (g = this.chartData[g]) {
                        ...
                        if (b.oneBalloonOnly && b.valueBalloonsEnabled) {
                            ...
                            for (h = d.length - 1; 0 <= h; h--)
                                if (f = d[h], f.balloon.enabled && f.showBalloon && !f.hidden) {
                                    ...
                                    if (b.showNextAvailable && isNaN(l.y) && (l = this.getNextItem(l), !l))
                                        continue;
                                    l = l.y;
                        ...
                        if (this.prevCursorItem != g || k != this.prevMostCloseGraph) {
                            ...
                            for (h = 0; h < d.length; h++) {
                                f = d[h];
								//console.log("f=", f);
								//console.log("f=", f.accessibleLabel);
								//console.log("set=", this.set);
								
								this.eventTest(this.set);
                                l = f.valueAxis.id;
                                l = g.axes[l].graphs[f.id];
        ...
        handleCursorPanning: function (a) {
            ...
            if (this.rotate) {
                ...
                isNaN(f) && (f = g, a = !0), l = this.endY0, c = this.startY0, k = l - c, l += k * g, m = k, a || (m = 0), a = e.fitToBounds(c + k * f, 0, 1 - m);
            ...
            if (b.zoomable && (0 < Math.abs(d) || 0 < Math.abs(h)))
                if (c.parseDates && !c.equalSpacing) {
                    if (f = this.startTime0, g = this.endTime0, c = g - f, h *= c, k = this.firstTime, l = this.lastTime, m = c, a || (m = 0), a = Math.round(e.fitToBounds(f - c * d, k, l - m)), h = Math.round(e.fitToBounds(g - h, k + m, l)), this.startTime != a || this.endTime != h)
                        n = {
                            chart: this,
                            target: b,
                            type: "zoomed",
                    ...
                    (c = 0), a = e.fitToBounds(f - d, 0, k - c), c = e.fitToBounds(g - h, c, k), this.start != a || this.end != c)
                    this.skipZoomed = !0, b.fire({
                        chart: this,
                        target: b,
                        type: "zoomed",
                        start: a,
    ...
    e.Cuboid = e.Class({
        ...
        draw: function () {
            ...
            if (0 < Math.abs(h) || 0 < Math.abs(f))
                if (isNaN(t))
                    z = q, q = e.adjustLuminosity(B,  - .2), q = e.adjustLuminosity(B,  - .2), w = e.polygon(b, [0, h, d + h, d, 0], [0, f, f, 0, 0],
			...
			//console.log("3 draw");
			for (var i = 0; i < c.chartData.length; i++) {
				//console.log("i=", c.chartData[i].dataContext.visits);
				//this.clickEvent3(this.set, c.chartData[i].dataContext.visits);
			}
		...
		clickEvent3:function(a, b) {
			var c = this;
			///*
            a.mouseover(function (a) {
				console.log("clickEvent3");
            }).mouseout(function (a) {
                console.log("clickEvent3");
            }).mousedown(function (a) {
                //console.log("clickEvent3");
...
(function () {
    ...
    e.CategoryAxis = e.Class({
        ...
        draw: function () {
			...
			if (e.ifArray(a)) {
                ...
                if (this.parseDates && !this.equalSpacing) {
					this.lastTime = a[a.length - 1].time, this.maxTime = e.resetDateToMin(new Date(this.lastTime + 1.05 * u), this.minPeriod,
                        ...
                        for (q = Math.floor(f / l), t = this.chooseMinorFrequency(q), f = h, f / 2 == Math.round(f / 2) && f--, 0 > f && (f = 0), w = 0, this.widthField && (f = this.start, q = 1), this.end - f + 1 >= this.autoRotateCount && (this.labelRotationR = this.autoRotateAngle), b = f; b <= this.end + 2; b++) {
                            ...
                            if (r && !isNaN(t))
                                if (b / t == Math.round(b / t) || l)
                                    b / q == Math.round(b / q) || l || (this.gridAlpha = this.minorGridAlpha, n = void 0);
                                else
                                    continue;
                ...
                for (b = k = 0; b < this.data.length; b++)
                    if (t = this.data[b])
                        this.parseDates && !this.equalSpacing ? (m = t.time, d = this.cellWidth, "MM" == this.minPeriod &&
		...
		clickEvent4: function(a) {
			
			var c = this;
            a.mouseover(function (a) {
				console.log("clickEvent4");
            }).mouseout(function (a) {
                console.log("clickEvent4");
            }).mousedown(function (a) {
                console.log("clickEvent4");
