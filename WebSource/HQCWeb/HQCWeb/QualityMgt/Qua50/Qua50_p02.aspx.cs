...
{
    public partial class Qua50_p02 : System.Web.UI.Page
    {
        ...
        {
            //수정일경우
            lbWorkName.Text = Dictionary_Data.SearchDic("DETAIL", bp.g_language); // 수정
            ...
            lbInboundDt.Text = Dictionary_Data.SearchDic("INBOUND_DT", bp.g_language);
            lbCompleteNo.Text = Dictionary_Data.SearchDic("COMPLETE_NO", bp.g_language);
        ...
        protected void GetData()
        {
            ...
            strDBName = "GPDB";
            strQueryID = "Qua50Data.Get_EOInfo";

            FW.Data.Parameters sParam = new FW.Data.Parameters();
            sParam.Add("PLANT_CD", strSplitValue[0].ToString());
            sParam.Add("NO", strSplitValue[1].ToString());
            ...
            if (ds.Tables.Count > 0)
            {
                ...
                {
                    GetDDL(ds.Tables[0].Rows[0]["PART_NO"].ToString(), ds.Tables[0].Rows[0]["SHOP_CD"].ToString(), ds.Tables[0].Rows[0]["LINE_CD"].ToString());

                    lbGetRegDt.Text = ds.Tables[0].Rows[0]["PROD_DT"].ToString();
                    ddlEo4mFlag.Text = ds.Tables[0].Rows[0]["EO4M_FLAG"].ToString();
                    lbGetPartNo.Text = ds.Tables[0].Rows[0]["PART_NO"].ToString();
                    ddlShopCd.Text = ds.Tables[0].Rows[0]["SHOP_CD"].ToString();
                    ddlLineCd.Text = ds.Tables[0].Rows[0]["LINE_CD"].ToString();
                    ddlCarType.Text = ds.Tables[0].Rows[0]["CAR_TYPE"].ToString();
                    txtEoNo.Text = ds.Tables[0].Rows[0]["EO_NO"].ToString();
                    txtRemark.Text = ds.Tables[0].Rows[0]["REMARK"].ToString();
                    txtModRemark.Text = ds.Tables[0].Rows[0]["MOD_REMARK"].ToString();
                    txtDate.Text = ds.Tables[0].Rows[0]["INBOUND_DT"].ToString();
                    txtCompleteNo.Text = ds.Tables[0].Rows[0]["COMPLETE_NO"].ToString();
                    ddlEo4mFlag.Enabled = false;
                    ...
                    for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                    {
                        ...
                        if(ds.Tables[0].Rows[i]["ORG_FILE_NAME"].ToString().Length > 0)
                        {
                            ...
                            //strReplaceFileName = ds.Tables[0].Rows[i]["ORG_FILE_NAME"].ToString().Replace(strFileExt, "").Replace(".", "_").Replace(" ", "_");
                            strReplaceFileName = Regex.Replace(ds.Tables[0].Rows[i]["ORG_FILE_NAME"].ToString(), @"[^a-zA-Z0-9가-힣-_:.]", "_");

                            if (strFileExt.ToLower() == "png" || strFileExt.ToLower() == "gif" || strFileExt.ToLower() == "jpg")
                            {
                                strICon = "image";
                            }

                            if (strFileExt.ToLower() == "ppt" || strFileExt.ToLower() == "pptx")
                            {
                                strICon = "powerpoint";
                            }
            ...
            {
                strScript = " alert('정보가 존재하지 않습니다. 관리자에게 문의바립니다.'); parent.fn_ModalReloadCall('Qua50'); parent.fn_ModalCloseDiv(); ";
        ...
        protected void btnModify_Click(object sender, EventArgs e)
        {
            ...
            strRtn = SetFileUplad(strSplitValue[0].ToString(), strSplitValue[1].ToString());
            ...
            if (strRtn.Equals("1"))
            {
                ...
                strDBName = "GPDB";
                strQueryID = "Qua50Data.Upt_EOInfo";
                ...
                sParam.Add("COMPLETE_NO", txtCompleteNo.Text);
                sParam.Add("INBOUND_DT", txtDate.Text.Replace("-", ""));
                ...
                if (iRtn == 1)
                {
                    ...
                    strScript = " alert('정상수정 되었습니다.');  parent.fn_ModalReloadCall('Qua50'); parent.fn_ModalCloseDiv(); ";
                    ClientScript.RegisterStartupScript(typeof(string), Guid.NewGuid().ToString(), strScript, true);
                }
                else
                {
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            strDBName = "GPDB";
            strQueryID = "Qua50Data.Del_EOInfo";
            ...
            if (iRtn == 1)
            {
                ...
                {
                    string File_Path = System.Configuration.ConfigurationManager.AppSettings.Get("FILE_PATH") + "Qua50\\";
                ...
                strScript = " alert('정상삭제 되었습니다.');  parent.fn_ModalReloadCall('Qua50'); parent.fn_ModalCloseDiv(); ";
                ClientScript.RegisterStartupScript(typeof(string), Guid.NewGuid().ToString(), strScript, true);
            }
            else
            {
        ...
        #region GetDDL
        private void GetDDL(string partNo, string shopCd, string lineCd)
        {
            DataSet ds = new DataSet();
            ...
            if (ds.Tables.Count > 0)
            {
                ...
                {
                    ddlLineCd.Items.Add(new ListItem(ds.Tables[5].Rows[i]["id"].ToString(), ds.Tables[5].Rows[i]["title"].ToString()));
                }
            }
        }
        #endregion
        ...
        public static string SetFileInfoDel(string sParams, string sParams2, string sParams3)
        {
            ...
            //세션에서 키값 추출
            string strPVal = HttpContext.Current.Session["Qua50PopValue"]?.ToString();
            ...
            strDBName = "GPDB";
            strQueryID = "Qua50Data.Set_FileInfoDel";
            ...
            if (iRtn == 1)
            {
                string fileName = strFilePath + "\\Qua50\\" + strPath[0].ToString() + "\\" + strChgFileName[0].ToString();
        ...
        #region SetFileUplad
        public string SetFileUplad(string plantCd, string no)
        {
            string strRtn = string.Empty;
            int iRow = 0;
            int iRtn = 0;
                ...
                try
                {
                    if (arrFileName.Length > 0)
                    {
                        ...
                        if (iCnt > 0)
                        {
                            ...
                            {
                                strDBName = "GPDB";
                                strQueryID = "Qua50Data.Set_FileInfo";
