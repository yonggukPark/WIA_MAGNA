<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage/Site.Master" AutoEventWireup="true" CodeBehind="Info38.aspx.cs" Inherits="HQCWeb.InfoMgt.Info38.Info38" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
    <script type="text/javascript">
        ...
        function fn_Register() {
            fn_OpenPop('InfoMgt/Info38/Info38_p01.aspx', $("#MainContent_hidWidth").val(), $("#MainContent_hidHeight").val());
        }

        function fn_Modify(pkCode) {
            fn_PostOpenPop(pkCode, 'InfoMgt/Info38/Info38_p02.aspx', $("#MainContent_hidWidth").val(), $("#MainContent_hidHeight").val());
        }

        function fn_Copy() {
            if (currentValue != undefined)
                fn_PostOpenPop(currentValue, 'InfoMgt/Info38/Info38_p03.aspx', $("#MainContent_hidWidth").val(), $("#MainContent_hidHeight").val());
            else
                alert("먼저 복사할 행을 선택해주세요.");
        }

        function fn_Register_2() {
            if (currentValue != undefined && workValue != "N" )
                fn_PostOpenPop(currentValue, 'InfoMgt/Info38/Info38_p04.aspx', '600', '500');
            else if (workValue != "Y")
                alert("연결 불가능한 데이터입니다. 공정작업순서를 확인하세요.");
            else
                alert("먼저 연결할 스캔코드를 선택해주세요.");
        }

        function fn_Modify_2(pkCode) {
            fn_PostOpenPop(pkCode, 'InfoMgt/Info38/Info38_p05.aspx', '600', '500');
        }
        ...
        function settingGrid(_val) {
            ...
            gridView.onCellClicked = function (grid, clickData) {
                ...
                workValue = dataProvider.getValue(current.dataRow, "WORK_FLAG");
                currentValue = value;

                if (clickData.cellType !== "data") {
                    return; // 데이터 셀이 아니면 이벤트 중단
                }
                if (clickData.column == 'SCAN_CD') {
                    fn_Modify(value);
                } else if (clickData.column == 'COLUMN_SEQ') {
                    var chk = dataProvider.getValue(current.dataRow, "MATCH_FLAG");
                    if(chk == 'Yes')
                        fn_Modify_2(value);
                    else 
                        alert("매칭된 데이터가 없습니다.");
            ...
            gridView.columnByName("SCAN_CD").styleName = "grid-primary-column"
            gridView.columnByName("COLUMN_SEQ").styleName = "grid-primary-column"

            gridView.columnByName("SCAN_NM").styleName = "string-column";
            gridView.columnByName("MATCH_NM").styleName = "string-column";
            gridView.columnByName("CHK_DATA").styleName = "string-column";
            gridView.columnByName("BRCD_CHECK").styleName = "string-column";
            
            gridView.columnByName("REMARK1").styleName = "string-column";
            gridView.columnByName("REMARK2").styleName = "string-column";

            gridView.columnByName("KEY_HID").visible = false;
            gridView.columnByName("WORK_FLAG").visible = false;
    ...
    <asp:HiddenField ID="hidWidth" runat="server" Value="1400" />
    <asp:HiddenField ID="hidHeight" runat="server" Value="600" />

    <div class="contents" tabindex="0">
        <!--// Title + Win_Btn -->
        <div class="title">
            <h3><asp:Label ID="lbTitle" runat="server">Info38</asp:Label></h3>
            <div class="al-r">
                ...
                <input type="button" value="Match" onclick="javascript:fn_Register_2();" ID="btnSubNew" runat="server" visible="false" />
                <input type="button" value="Copy" onclick="javascript:fn_Copy();" ID="btnCopy" runat="server" visible="false" />
                <input type="button" value="Excel" id="btnExcel" runat="server" visible="false" onclick="fn_excelExport('Info38'); return false;" />
            </div>
        </div>
        <!-- Title + Win_Btn //-->
        <!--// Grid Search -->
                        ...
                        <tr>
                            <td>
                                <div class="toolbar">
                                    ...
                                    <input type="button" value="Set" id="btnSet" onclick="getCol('Info38'); return false;" style="display:none; margin-top:10px; float:left" />
