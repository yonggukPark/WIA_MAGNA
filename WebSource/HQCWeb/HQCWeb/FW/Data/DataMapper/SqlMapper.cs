...
using HQCWeb.FW.Data.DataMapper.DataExchange;
...
namespace HQCWeb.FW.Data.DataMapper
{
	/// <summary>
	...
	public class SqlMapper : ISqlMapper
	{
		...
		public ISqlMapSession BeginTransaction() 
		{
			if (_sessionStore.LocalSession != null) 
			{
                _sessionStore.LocalSession.BeginTransaction();
		...
		public ISqlMapSession BeginTransaction(IsolationLevel isolationLevel)
		{
			if (_sessionStore.LocalSession != null)
            {
                _sessionStore.LocalSession.BeginTransaction(isolationLevel);
                return _sessionStore.LocalSession;
                //throw new DataMapperException("SqlMap could not invoke BeginTransaction(). A Transaction is already started. Call CommitTransaction() or RollbackTransaction first.");
			}
            ISqlMapSession session = CreateSqlMapSession();
		...
		public ISqlMapSession BeginTransaction(string connectionString, IsolationLevel isolationLevel)
		{
			if (_sessionStore.LocalSession != null)
            {
                _sessionStore.LocalSession.BeginTransaction(connectionString, isolationLevel);
                return _sessionStore.LocalSession;
                //throw new DataMapperException("SqlMap could not invoke BeginTransaction(). A Transaction is already started. Call CommitTransaction() or RollbackTransaction first.");
			}
            ISqlMapSession session = CreateSqlMapSession(connectionString);
        ...
        public DataSet QueryForDataSet(string statementName, object parameterObject)
        {
            ...
            try
            {
                ...
                {
                    IDataReader reader;
                    reader = request.IDbCommand.ExecuteReader();
                ...
                dataTable = new DataTable("ErrorLog");
                dataTable.Columns.Add("Status");
                dataTable.Columns.Add("Msg");

                DataRow row = dataTable.NewRow();
                row["Status"] = "ERROR";
                row["Msg"] = e.Message.ToString().Replace("'", "");
                ...
                htParameter = (Hashtable)parameterObject;
                ...
                strQueryID = statementName;
                strErrorLog = e.Message.ToString() + "\r\n" + e.StackTrace.ToString();

                Parameters sParams = new Parameters();

                if (strMenuID == "") {
                    sParams.Add("LogFlag", "N");
                }

                sParams.Add("MENU_ID",      strMenuID);
                sParams.Add("QUERY_ID",     strQueryID);
                sParams.Add("ERROR_LOG",    strErrorLog);
                
                Biz.SystemManagement.ErrorLogMgt biz = new Biz.SystemManagement.ErrorLogMgt();

                strDBName = "GPDB";
                strQueryID = "ErrorLogData.Set_ERRORLog_Data";

                biz.SetErrorLogInfo(strDBName, strQueryID, sParams);
            }
            finally
            {
                if (isSessionLocal)
