...
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <script type="text/javascript">
        ...
        function createGrid(_val) {
            ...
            field.forEach(item => {
                if (item.fieldName == 'RES_OK_SUM' || item.fieldName.substring(0, 6) == 'RES_TM') {
        ...
        function settingGrid(_val) {
            ...
            gridView.onCellClicked = function (grid, clickData) {
                ...
                if (clickData.column.substring(0, 6) == 'RES_TM' || clickData.column == 'RES_OK_SUM') {
                    var current = gridView.getCurrent();
                    var value = dataProvider.getValue(current.dataRow, "KEY_HID");
                    var seq = dataProvider.getValue(current.dataRow, "PROD_DT") + ((clickData.column.substring(0, 6) == 'RES_TM') ? clickData.column.substring(6, 8) : '');
                    fn_Chk(value, seq);
                }
            }
            ...
            gridView.columnByName("RES_OK_SUM").styleCallback = f;
            gridView.columnByName("LINE_CD").styleName = "string-column";
            gridView.columnByName("PART_DESC").styleName = "string-column";

            for (var i = 0; i < 24; i++) {
                gridView.columnByName("RES_TM" + (i).toString().padStart(2, '0')).styleName = "number-column";
            }
            gridView.columnByName("RES_OK_SUM").styleName = "number-column";

            gridView.columnByName("KEY_HID").visible = false;
            gridView.columnByName("P_LINE_CD").visible = false;
                        ...
                        <tr>
                            <td>
                                <div class="toolbar">
                                    ...
                                    <input type="button" value="Set" id="btnSet" onclick="getCol('Res02'); return false;" style="display:none; margin-top:10px; float:left" />
